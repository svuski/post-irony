...
function renderMetrics(){
  ...
  const exams=data.exams.filter(e=>e.date&&daysUntil(e.date)>=0).sort((a,b)=>a.date.localeCompare(b.date));
  mActive.textContent=active;
  mDeadline.textContent=deadlines[0]?`D-${daysUntil(deadlines[0].deadline)}`:'—';
  mDeadlineSub.textContent=deadlines[0]?`${deadlines[0].company} · ${fmt(deadlines[0].deadline)}`:'등록된 마감 없음';
  mExam.textContent=exams[0]?`D-${daysUntil(exams[0].date)}`:'—';
  mExamSub.textContent=exams[0]?`${exams[0].name} · ${fmt(exams[0].date)}`:'등록된 시험 없음';
}
...
function renderExams(){
  const list=[...data.exams].sort((a,b)=>(a.date||'9999').localeCompare(b.date||'9999'));
  if(!list.length){examList.innerHTML='<div class="empty">시험 없음.</div>';return}
  examList.innerHTML=list.map(e=>{const d=e.date?daysUntil(e.date):null;const dtext=d===0?'D-DAY':d>0?`D-${d}`:d!==null?'DONE':'—';return `<div class="exam-row exam-schedule-row">
    <div class="exam-row-head"><div><div class="exam-name">${esc(e.name)}</div>${e.goal?`<div class="muted tiny">목표 ${esc(e.goal)}</div>`:''}</div><div class="dday">${dtext}</div></div>
    <div class="exam-dates">
      <div class="exam-date-block"><span class="exam-date-label">접수</span><strong>${fmt(e.registrationDate)}</strong>${e.fee?`<span class="tiny muted">${esc(e.fee)}</span>`:''}</div>
      <div class="exam-date-block"><span class="exam-date-label">시험</span><strong>${fmt(e.date)}</strong></div>
      <div class="exam-date-block"><span class="exam-date-label">결과</span><strong>${fmt(e.resultDate)}</strong></div>
    </div>
    <div class="exam-row-foot">${e.notes?`<span class="muted tiny">${esc(e.notes)}</span>`:'<span></span>'}<button class="btn ghost tiny" data-edit-exam="${esc(e.id)}">수정</button></div>
  </div>`}).join('')
}
...
addExamBtn.onclick=()=>{fillForm(examForm,{id:''});examDialog.showModal()};
resumeVaultBtn.onclick=()=>resumeDialog.showModal();
