...
function openHorizon(key){currentHorizon=key;const item=HORIZONS.find(([k])=>k===key);if(!item)return;horizonDialogTitle.textContent=item[1];horizonTrail.textContent=TRAILS[key]||'';horizonDialogBody.innerHTML=`<div class="popup-areas">${Object.entries(areas).map(([area,a])=>renderArea(key,area,a)).join('')}</div>`;horizonDialog.showModal()}
function refreshOpenDialogs(){if(horizonDialog.open)openHorizon(currentHorizon);if(inboxDialog.open)renderInbox();if(calendarDialog.open)renderCalendarLarge()}
