...
const horizonDialogTitle=document.getElementById('horizonDialogTitle');
const horizonDialogBody=document.getElementById('horizonDialogBody');
const horizonTrail=document.getElementById('horizonTrail');
...
function openHorizon(key){const item=HORIZONS.find(([k])=>k===key);if(!item)return;const title=item[1];horizonDialogTitle.textContent=title;const tiny=HORIZONS.filter(([k])=>k!==key).map(([,t])=>t).join(' · ');horizonTrail.textContent=tiny;horizonDialogBody.innerHTML=`<div class="popup-areas">${Object.entries(areas).map(([area,a])=>renderArea(key,area,a)).join('')}</div>`;horizonDialog.showModal()}
...
[taskDialog,horizonDialog].forEach(d=>d.addEventListener('click',e=>{if(e.target===d)d.close()}));
taskForm.addEventListener('submit',e=>{e.preventDefault();if(!taskForm.reportValidity())return;const f=Object.fromEntries(new FormData(taskForm));f.focus=taskForm.elements.focus.checked?'yes':'';if(f.focus==='yes'){const others=data.tasks.filter(t=>t.focus==='yes'&&!t.done&&t.id!==f.id);if(others.length>=3){alert('FOCUS 3는 최대 3개까지 표시할 수 있어.');return}}if(f.id){Object.assign(data.tasks.find(t=>t.id===f.id),f)}else data.tasks.push({...f,id:uid(),done:false});taskDialog.close();save()});

document.body.addEventListener('change',e=>{const id=e.target.dataset.toggle;if(!id)return;const t=data.tasks.find(x=>x.id===id);if(t){t.done=e.target.checked;save();if(horizonDialog.open){const key=taskForm.elements.horizon.value;void key}}});
document.body.addEventListener('click',e=>{const horizon=e.target.closest('[data-open-horizon]');if(horizon){openHorizon(horizon.dataset.openHorizon);return}const toggle=e.target.closest('[data-toggle-section]');if(toggle){document.getElementById(toggle.dataset.toggleSection)?.classList.toggle('open');toggle.classList.toggle('expanded');return}const cal=e.target.closest('[data-date]');if(cal){document.querySelectorAll('.cal-day.selected').forEach(x=>x.classList.remove('selected'));cal.classList.add('selected');showAgenda(cal.dataset.date);return}const edit=e.target.closest('[data-edit]');const id=edit?.dataset.edit;if(!id)return;const t=data.tasks.find(x=>x.id===id);if(!t)return;taskForm.reset();Object.entries(t).forEach(([k,v])=>{if(taskForm.elements[k]&&k!=='focus')taskForm.elements[k].value=v??''});taskForm.elements.focus.checked=t.focus==='yes';taskDialog.showModal()});
