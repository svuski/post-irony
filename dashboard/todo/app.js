...
function renderHorizons(){
  horizonsEl.innerHTML=HORIZONS.map(([key,title])=>{
...
function renderCalendar(){
  ...
  calendarTitle.textContent=new Intl.DateTimeFormat('en-US',{month:'long',year:'numeric'}).format(calendarDate).toUpperCase();
  const first=new Date(year,month,1),last=new Date(year,month+1,0),today=new Date();
  const cells=[];
  for(let i=0;i<first.getDay();i++)cells.push('<span class="cal-empty"></span>');
  for(let d=1;d<=last.getDate();d++){
    const iso=`${year}-${String(month+1).padStart(2,'0')}-${String(d).padStart(2,'0')}`;
    const related=data.tasks.filter(t=>t.due===iso);
    const isToday=today.getFullYear()===year&&today.getMonth()===month&&today.getDate()===d;
    cells.push(`<button class="cal-day ${isToday?'today':''} ${related.length?'has-task':''}" data-date="${iso}"><span>${d}</span>${related.length?`<i>${related.length}</i>`:''}</button>`)
  }
...
document.body.addEventListener('click',e=>{
  ...
  if(toggle){document.getElementById(toggle.dataset.toggleSection)?.classList.toggle('open');toggle.classList.toggle('expanded');return}
  const cal=e.target.closest('[data-date]');
  if(cal){document.querySelectorAll('.cal-day.selected').forEach(x=>x.classList.remove('selected'));cal.classList.add('selected');showAgenda(cal.dataset.date);return}
  const edit=e.target.closest('[data-edit]');
  const id=edit?.dataset.edit;
  if(!id)return;
  const t=data.tasks.find(x=>x.id===id);if(!t)return;
  taskForm.reset();Object.entries(t).forEach(([k,v])=>{if(taskForm.elements[k]&&k!=='focus')taskForm.elements[k].value=v??''});taskForm.elements.focus.checked=t.focus==='yes';taskDialog.showModal()
});
