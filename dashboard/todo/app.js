...
function openHorizon(key){const item=HORIZONS.find(([k])=>k===key);if(!item)return;horizonDialogTitle.textContent=item[1];horizonTrail.textContent=TRAILS[key]||'';horizonDialogBody.innerHTML=`<div class="popup-areas">${Object.entries(areas).map(([area,a])=>renderArea(key,area,a)).join('')}</div>`;horizonDialog.showModal()}
