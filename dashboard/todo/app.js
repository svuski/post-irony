...
function calendarCells(targetLarge=false){const year=calendarDate.getFullYear(),month=calendarDate.getMonth();const first=new Date(year,month,1),last=new Date(year,month+1,0),today=new Date();const cells=[];for(let i=0;i<first.getDay();i++)cells.push('<span class="cal-empty"></span>');for(let d=1;d<=last.getDate();d++){const iso=`${year}-${String(month+1).padStart(2,'0')}-${String(d).padStart(2,'0')}`;const hasEvent=data.events.some(e=>e.date===iso);const isToday=today.getFullYear()===year&&today.getMonth()===month&&today.getDate()===d;cells.push(`<button class="cal-day ${targetLarge?'cal-day-large':''} ${isToday?'today':''} ${selectedDate===iso?'selected':''}" data-date="${iso}"><span>${d}</span>${hasEvent?'<span class="event-dots"><b class="event-dot"></b></span>':''}</button>`)}return cells.join('')}
...
function renderEventRow(e,large=false){return `<div class="calendar-event-row ${large?'large':''}"><span class="event-dot"></span><div><strong>${esc(e.text)}</strong>${large&&e.notes?`<p>${esc(e.notes)}</p>`:''}</div>${large?`<div class="event-row-actions"><button data-edit-event="${e.id}">EDIT</button><button data-delete-event="${e.id}">DELETE</button></div>`:''}</div>`}
...
function openEvent(date=selectedDate||isoToday()){eventForm.reset();eventForm.elements.id.value='';eventForm.elements.date.value=date;deleteEventBtn.style.display='none';eventDialog.showModal()}
function editEvent(e){eventForm.reset();eventForm.elements.id.value=e.id;eventForm.elements.text.value=e.text||'';eventForm.elements.date.value=e.date||isoToday();eventForm.elements.notes.value=e.notes||'';deleteEventBtn.style.display='inline-flex';eventDialog.showModal()}
